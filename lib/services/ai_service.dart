...
class AIService {
  ...
  // API keys - set these to use real AI (Claude, OpenAI, or Gemini)
  String? _claudeApiKey;
  String? _openAiApiKey;
  String? _geminiApiKey;
  
  // Which AI provider to use ('claude', 'openai', 'gemini', or 'local' for rule-based)
  String _provider = 'local';
  
  // Store last API error for UI display
  ...
  void setOpenAiApiKey(String key) {
    _openAiApiKey = key;
    if (key.isNotEmpty) {
      // Only auto-set provider if not explicitly set to something else
      if (_provider == 'local') {
        _provider = 'openai';
      }
    }
  ...
  void setGeminiApiKey(String key) {
    _geminiApiKey = key;
    debugPrint('Gemini API key set: ${key.isNotEmpty ? 'Present' : 'Empty'}');
    if (key.isNotEmpty) {
      // Only auto-set provider if not explicitly set to something else
      if (_provider == 'local') {
        _provider = 'gemini';
        debugPrint('Auto-set provider to: gemini');
      }
    }
  }
  
  void setProvider(String provider) {
    _provider = provider; // 'claude', 'openai', 'gemini', or 'local'
    debugPrint('AI Provider explicitly set to: $_provider');
    debugPrint('Current API keys - Claude: ${_claudeApiKey != null && _claudeApiKey!.isNotEmpty}, OpenAI: ${_openAiApiKey != null && _openAiApiKey!.isNotEmpty}, Gemini: ${_geminiApiKey != null && _geminiApiKey!.isNotEmpty}');
  }
  
  // Clear all API keys (useful when switching providers)
  void clearApiKeys() {
    _claudeApiKey = null;
    _openAiApiKey = null;
    _geminiApiKey = null;
  ...
  // Generate therapist-style response
  Future<String> generateTherapistResponse(
    ...
    debugPrint('Claude key: ${_claudeApiKey != null && _claudeApiKey!.isNotEmpty}');
    debugPrint('OpenAI key: ${_openAiApiKey != null && _openAiApiKey!.isNotEmpty}');
    debugPrint('Gemini key: ${_geminiApiKey != null && _geminiApiKey!.isNotEmpty}');
    
    // Use local rule-based responses if no API keys are set
    if (_provider == 'local' || 
        (_provider == 'claude' && (_claudeApiKey == null || _claudeApiKey!.isEmpty)) ||
        (_provider == 'openai' && (_openAiApiKey == null || _openAiApiKey!.isEmpty)) ||
        (_provider == 'gemini' && (_geminiApiKey == null || _geminiApiKey!.isEmpty))) {
    ...
    try {
      ...
      // Check Gemini FIRST (since it's the new default)
      if (_provider == 'gemini' && _geminiApiKey != null && _geminiApiKey!.isNotEmpty) {
        debugPrint('Attempting to use Google Gemini API...');
        try {
          _lastApiError = null; // Clear previous error
          final response = await _generateGeminiResponse(userMessage, emotionContext);
          debugPrint('Gemini API call successful!');
          return response;
        } catch (e) {
          debugPrint('Gemini API error, falling back to rule-based: $e');
          // Store error for UI to display
          _lastApiError = e.toString();
          // Don't throw - return fallback instead
          return _generateRuleBasedResponse(userMessage, recentEmotions);
        }
      } else if (_provider == 'claude' && _claudeApiKey != null && _claudeApiKey!.isNotEmpty) {
        try {
          return await _generateClaudeResponse(userMessage, emotionContext);
        } catch (e) {
          debugPrint('Claude API error, falling back to rule-based: $e');
          return _generateRuleBasedResponse(userMessage, recentEmotions);
        }
      } else if (_provider == 'openai' && _openAiApiKey != null && _openAiApiKey!.isNotEmpty) {
        debugPrint('Attempting to use OpenAI API...');
        try {
          _lastApiError = null; // Clear previous error
          final response = await _generateOpenAiResponse(userMessage, emotionContext);
          debugPrint('OpenAI API call successful!');
          return response;
        } catch (e) {
          debugPrint('OpenAI API error, falling back to rule-based: $e');
          // Store error for UI to display
          _lastApiError = e.toString();
          // Don't throw - return fallback instead
          return _generateRuleBasedResponse(userMessage, recentEmotions);
  ...
  Future<String> _generateClaudeResponse(String userMessage, String emotionContext) async {
    final response = await http.post(
      ...
      body: jsonEncode({
        ...
        'messages': [
          {
            'role': 'user',
            'content': '''You are an empathetic mental health companion. Provide supportive, non-judgmental responses.
...
Respond with warmth, empathy, and practical coping strategies. Keep responses concise (2-3 paragraphs).'''
  ...
  Future<String> _generateOpenAiResponse(String userMessage, String emotionContext) async {
    debugPrint('Calling OpenAI API...');
    debugPrint('API Key present: ${_openAiApiKey != null && _openAiApiKey!.isNotEmpty}');
    ...
    try {
      final requestBody = {
        ...
        'messages': [
          {
            'role': 'system',
            'content': 'You are an empathetic mental health companion. Provide supportive, non-judgmental responses with warmth, empathy, and practical coping strategies. Keep responses concise (2-3 paragraphs).'
          },
          {
            'role': 'user',
            'content': '''Context: $emotionContext
...
Please provide a supportive, empathetic response.'''
      ...
      final response = await http.post(
        ...
        onTimeout: () {
          throw Exception('Request timeout: OpenAI API took too long to respond');
        },
      );

      debugPrint('OpenAI response status: ${response.statusCode}');
      debugPrint('OpenAI response body: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}...');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['choices'] != null && 
          ...
          throw Exception('Invalid response format from OpenAI API: Missing choices or content');
        }
      } else {
        ...
        try {
          ...
          if (errorData['error'] != null) {
            ...
            errorMessage = 'OpenAI API error: ${error['message'] ?? error['type'] ?? errorMessage}';
      ...
      } else if (errorStr.contains('401') || errorStr.contains('Unauthorized') || errorStr.contains('Invalid API key')) {
        throw Exception('API key error: Invalid or expired API key. Please check your OpenAI API key in main.dart');
      ...
      } else if (errorStr.contains('quota') || errorStr.contains('insufficient_quota') || errorStr.contains('exceeded')) {
        throw Exception('Quota exceeded: Your OpenAI account has run out of credits. Please add billing/credits to your OpenAI account at https://platform.openai.com/account/billing or switch to local AI mode.');
      } else if (errorStr.contains('timeout')) {
        throw Exception('Request timeout: OpenAI API took too long to respond. Please try again.');
      } else {
  ...
  Future<String> _generateGeminiResponse(String userMessage, String emotionContext) async {
    debugPrint('Calling Google Gemini API...');
    debugPrint('API Key present: ${_geminiApiKey != null && _geminiApiKey!.isNotEmpty}');
    
    try {
      // Gemini API endpoint
      final model = 'gemini-1.5-flash'; // or 'gemini-1.5-pro' for better quality
      final url = Uri.parse('$geminiApiBaseUrl/models/$model:generateContent?key=$_geminiApiKey');
        ...
        'contents': [
          {
            'parts': [
              {
                'text': '''You are an empathetic mental health companion. Provide supportive, non-judgmental responses with warmth, empathy, and practical coping strategies. Keep responses concise (2-3 paragraphs).
...
Please provide a supportive, empathetic response.'''
      ...
      debugPrint('Gemini response body: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}...');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['candidates'] != null && 
          ...
          debugPrint('Invalid response format: ${data.toString()}');
          throw Exception('Invalid response format from Gemini API: Missing candidates or content');
        }
      } else {
        // Parse error response
        ...
        try {
          ...
          if (errorData['error'] != null) {
            final error = errorData['error'];
            errorMessage = 'Gemini API error: ${error['message'] ?? error['status'] ?? errorMessage}';
      ...
      if (errorStr.contains('SocketException') || 
          ...
          errorStr.contains('Network is unreachable')) {
        throw Exception('Network error: Please check your internet connection');
      } else if (errorStr.contains('401') || errorStr.contains('403') || errorStr.contains('Unauthorized') || errorStr.contains('Invalid API key') || errorStr.contains('API key not valid')) {
        throw Exception('API key error: Invalid or expired API key. Please check your Gemini API key in main.dart');
      } else if (errorStr.contains('429') || errorStr.contains('rate limit') || errorStr.contains('RESOURCE_EXHAUSTED')) {
        throw Exception('Rate limit exceeded: Please wait a moment and try again');
      } else if (errorStr.contains('quota') || errorStr.contains('exceeded')) {
        throw Exception('Quota exceeded: Your Gemini API account has run out of quota. Please check your usage at https://aistudio.google.com/app/apikey');
      } else if (errorStr.contains('timeout')) {
  ...
  String _generateRuleBasedResponse(
    ...
    if (lowerMessage.contains('stress') || lowerMessage.contains('overwhelm') || 
        lowerMessage.contains('stressed') || lowerMessage.contains('pressure')) {
      String contextNote = '';
...
When you're ready, would it help to break down what's overwhelming you into smaller, manageable steps? Sometimes writing it down helps clarify what's truly urgent.''';
    ...
    if (lowerMessage.contains('anxious') || lowerMessage.contains('worried') || 
        lowerMessage.contains('anxiety') || lowerMessage.contains('panic') ||
...
Is there someone you trust who you could reach out to today? You don't have to go through this alone.''';
    ...
    if (lowerMessage.contains('procrastinat') || lowerMessage.contains('can\'t focus') || 
        lowerMessage.contains('unmotivated') || lowerMessage.contains('lazy') ||
...
What's one small thing you could accomplish in the next 10 minutes? Even tiny progress counts.''';
    ...
    // Sleep and tiredness
    if (lowerMessage.contains('tired') || lowerMessage.contains('sleep') || 
        lowerMessage.contains('exhausted') || lowerMessage.contains('fatigue')) {
...
What's been affecting your energy levels most?''';
    ...
    if (lowerMessage.contains('angry') || lowerMessage.contains('mad') || 
        lowerMessage.contains('frustrated') || lowerMessage.contains('irritated')) {
...
What would be most helpful for you right now?''';
    ...
    if (lowerMessage.contains('thank') || lowerMessage.contains('grateful') || 
...
Celebrating the good times is just as important as processing the difficult ones. What's contributing to these positive feelings?''';
    ...
    if (dominantEmotion != null) {
      contextIntro = 'I notice you\'ve been feeling $dominantEmotion recently. ';
    }
...
Would you like to talk more about what's on your mind, or would you prefer some coping strategies? I'm here for whatever you need.''';
  ...
  Future<Map<String, String>> generateDailyRecommendation(
    ...
    } else if (emotionCounts['anxious'] != null && emotionCounts['anxious']! > 2) {
      ...
      'nature': [
        {
          ...
          'description': 'Take a 15-minute walk in natural light. Notice the colors of the sky, the temperature of the air, the sounds around you. Natural light boosts serotonin and helps regulate your mood.',
          'imageUrl': 'https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=800',
        },
        {
          'title': 'Cloud Watching',
          'description': 'Spend 10 minutes lying down and watching clouds. Let your mind wander. This simple act of observation can be deeply meditative and grounding.',
      ...
      'art': [
        {
          ...
          'description': 'Grab a pen and paper. Don\'t think, just draw. Scribbles, shapes, patterns - whatever flows. Art doesn\'t need to be perfect to be healing.',
          'imageUrl': 'https://images.unsplash.com/photo-1513364776144-60967b0f800f?w=800',
        },
        {
          'title': 'Color Therapy',
          'description': 'Look at something colorful today - a painting, flowers, or even a coloring book. Colors have emotional resonance. What color calls to you right now?',
          'imageUrl': 'https://images.unsplash.com/photo-1456086272160-b28b0645b729?w=800',
        },
      ],
      'food': [
        {
          'title': 'Mindful Tea Ritual',
          'description': 'Brew your favorite tea. Notice the steam, the aroma, the warmth in your hands. Take slow sips and be fully present. This small ritual creates calm.',
          'imageUrl': 'https://images.unsplash.com/photo-1556679343-c7306c1976bc?w=800',
        },
        {
          'title': 'One Beautiful Meal',
          'description': 'Prepare one meal with intention today. Even something simple. Arrange it beautifully on your plate. Eating is nourishment for body and soul.',
      ...
      'music': [
        {
          ...
          'description': 'Put on a song you love and move your body for 3 minutes. Dance, sway, or just tap your feet. Music + movement = instant mood boost.',
          'imageUrl': 'https://images.unsplash.com/photo-1511379938547-c1f69419868d?w=800',
        },
        {
          'title': 'Soundtrack Your Day',
          'description': 'Create a playlist that matches how you want to feel - not how you currently feel. Let music guide your emotional journey today.',
      ...
      'exercise': [
        {
          ...
          'description': 'Spend 5 minutes stretching. Reach up, touch your toes, roll your shoulders. Your body holds tension - let it go, one stretch at a time.',
          'imageUrl': 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=800',
        },
        {
          'title': 'Breathing Exercise',
          'description': '4-7-8 breathing: Breathe in for 4 counts, hold for 7, exhale for 8. Repeat 4 times. This activates your parasympathetic nervous system and creates instant calm.',
  ...
  String generateCheckInPrompt(List<EmotionLog> recentLogs) {
    ...
    final hoursSinceLastLog = DateTime.now().difference(lastLog.dateTime).inHours;
